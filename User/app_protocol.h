/** @file app_protocol.h @brief BLE 应用层统一帧编解码 */
...
typedef struct { uint8_t command; uint16_t sequence; uint16_t payload_length; uint8_t payload[APP_PROTOCOL_MAX_PAYLOAD]; } app_protocol_frame_t;
...
uint8_t AppProtocol_Build(uint8_t command, uint16_t sequence, const uint8_t *payload,
                          uint8_t payload_length, uint8_t output[APP_PROTOCOL_MAX_FRAME]);
#endif
