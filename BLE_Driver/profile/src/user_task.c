...
{
    // Status
    /* 已断连的通知必须丢弃，不能保存到下一次连接。 */
    int msg_status = g_connnet_start ? KE_MSG_SAVED : KE_MSG_CONSUMED;
    uint8_t state = ke_state_get(dest_id);
  
    // check state of the task
    if(state == USER_IDLE && g_connnet_start)
    {
        ...
        else
        {
            ...
            user_env->operation->conidx  = app_env.conidx;

            // Stable measurement indication or intermediate measurement notification

            user_env->operation->op      = USER_DATA_NTF;
            //bluetooth notify handle
...
//      printf("user_gattc_write_req_ind_handler, idx = %x\r\n", att_idx);
    ...
    /* 先确认有效的 CCC 写入，再让 APP 触发首包。 */
    if (status == ATT_ERR_NO_ERROR && att_idx == USER_IDX_WRITE_NOTIFY_CFG)
    {
        ble_subscription_changed(co_read16p(param->value) == PRF_CLI_START_NTF);
    }
