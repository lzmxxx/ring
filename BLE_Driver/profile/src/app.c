...
uint16_t appm_pending_adv_interval = 0;
...
{
    if(!g_connnet_start && ke_state_get(TASK_APP) == APPM_READY)
    {
        ...
        /* 启动广播不代表断开；断开事件只从 GAPC_DISCONNECT_IND 发出。 */
        g_connnet_start = 0;
    }

}

bool appm_change_advertising_interval(uint16_t interval)
{
    if (g_connnet_start || ke_state_get(TASK_APP) != APPM_ADVERTISING || appm_pending_adv_interval != 0 ||
        interval < 0x0020 || interval > 0x4000)
    {
        return false;
    }
    /* 这里只请求停止；必须等 GAPM_ADV_* 的 CANCELED 完成事件再改参数。 */
    appm_pending_adv_interval = interval;
...
{
    if(!g_connnet_start && ke_state_get(TASK_APP) == APPM_ADVERTISING)
    {
        // Go in ready state
