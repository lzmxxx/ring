...
const struct attm_desc user_att_db[USER_IDX_NB] =
{
    ...
    [USER_IDX_WRITE_NOTIFY_CHAR]       =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
...
void user_exe_operation(void)
{
    ...
    /* 单连接工程只向 operation->conidx 投递一次。 */
    if(user_env->operation->cursor == 0)
    {
        struct gattc_send_evt_cmd *evt = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
                KE_BUILD_ID(TASK_GATTC, user_env->operation->conidx),
                prf_src_task_get(&user_env->prf_env, 0), gattc_send_evt_cmd,
                user_env->operation->length);
