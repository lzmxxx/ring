                                ...
                                ke_task_id_t const src_id)
{
    ...
    // Reset completed
        ...
        {
            if (appm_pending_adv_interval != 0)
            {
                if (param->status == GAP_ERR_CANCELED && ke_state_get(TASK_APP) == APPM_READY)
                {
                    app_env.adv_para.adv_int_min = appm_pending_adv_interval;
                    app_env.adv_para.adv_int_max = appm_pending_adv_interval;
                    /* 等取消完成后再提交新参数，最后清除 pending。 */
                    appm_start_advertising();
                }
                else if (param->status != GAP_ERR_NO_ERROR && ke_state_get(TASK_APP) == APPM_READY)
                {
                    /* 广播操作以错误结束：按原来的参数恢复广播。 */
                    appm_start_advertising();
                }
                appm_pending_adv_interval = 0;
            }
            else
            {
                ASSERT_INFO(param->status == GAP_ERR_NO_ERROR || param->status == GAP_ERR_CANCELED,
        ...
        case (GAPM_CANCEL):
        {
            /* CANCEL 命令本身成功不代表广播已停止，成功时继续等待 ADV 完成。 */
            if (param->status != GAP_ERR_NO_ERROR && appm_pending_adv_interval != 0)
            {
                appm_pending_adv_interval = 0;
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    app_env.rx_data.RxCurrentLen=0;
    appm_pending_adv_interval = 0;
    app_env.adv_para.adv_int_min = APP_ADV_FAST_INTERVAL;
    app_env.adv_para.adv_int_max = APP_ADV_FAST_INTERVAL;
    g_connnet_start = 0;
    appm_start_advertising();
    if (g_pcallback) {
